...
const reportReplyError = require('./report-error');
...
/**
 * The wrapper already registered for this callback, or undefined.
 *
 * Unlike $getSigHandler this never creates one: asking whether a callback is
 * registered must not be what registers it.
 */
DBusInterface.prototype.$findSigHandler = function (callback) {
  var index = this.$callbacks.indexOf(callback);
  return index === -1 ? undefined : this.$sigHandlers[index];
};
...
DBusInterface.prototype.addListener = DBusInterface.prototype.on = function (
  ...
  if (!bus.signals.listeners(signalFullName).length) {
    ...
    bus.addMatch(
      match,
      function (err) {
        // Reported rather than thrown: this callback runs from the connection's
        // 'message' listener, so a throw here escaped as an uncaughtException and
        // killed the host process. Callers cannot catch it - HAP-NodeJS wraps its
        // .on("StateChanged", ...) in try/catch precisely to survive a system where
        // AddMatch is refused (see HAP-NodeJS#993), and the throw defeated that.
        if (err) {
          reportReplyError(bus.connection, err);
          return;
        }
...
DBusInterface.prototype.removeListener = DBusInterface.prototype.off =
  function (signame, callback) {
    ...
    // Nothing of ours is attached to this signal, so there is no match rule of
    // ours to withdraw. That happens when AddMatch was refused - the listener
    // was never registered - and asking dbus to remove a rule it never
    // installed earns a MatchRuleNotFound on top of the original failure, on
    // the teardown path where it is least welcome. Looking the wrapper up
    // rather than calling $getSigHandler also stops an unknown callback
    // creating one on the way past.
    var handler = this.$findSigHandler(callback);
    if (!handler || bus.signals.listeners(signalFullName).indexOf(handler) === -1) {
      return;
    }

    bus.signals.removeListener(signalFullName, handler);
    if (!bus.signals.listeners(signalFullName).length) {
      ...
      bus.removeMatch(
        match,
        function (err) {
          // Same reasoning as addListener above. This one runs on the teardown path,
          // so a throw took the process down during shutdown rather than at startup.
          if (err) {
            reportReplyError(bus.connection, err);
            return;
          }
