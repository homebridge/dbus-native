          ...
          function (methodReturnResult) {
            ...
            if (
              ...
              if (Array.isArray(methodReturnResult)) {
                ...
                while (characterCounter < resultSignature.length) {
                  var character = resultSignature[characterCounter];
                  if (character === 'a') { characterCounter++; continue; }
                  if (character === '(' || character === '{') {
                    var close = character === '(' ? ')' : '}';
                    var depth = 1;
                    characterCounter++;
                    while (characterCounter < resultSignature.length && depth > 0) {
                      if (resultSignature[characterCounter] === character) depth++;
                      else if (resultSignature[characterCounter] === close) depth--;
                      characterCounter++;
                    }
                    typeCount++;
                    continue;
                ...
                isMultipleReturns = typeCount > 1 && methodReturnResult.length === typeCount;
              }
              methodReturnReply.body = isMultipleReturns ? methodReturnResult : [methodReturnResult];
            }
            try {
