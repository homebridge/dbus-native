...
module.exports = function bus(conn, opts) {
  ...
  this.invoke = function (msg, callback) {
    // A bus whose Hello was refused has no name and was never admitted to the
    // bus, so every call made on it fails somewhere further on - with an error
    // about that call rather than about the refusal that actually caused it.
    // Say which it was, once, at the point of use.
    if (self.helloError) {
      return (
        callback &&
        callback(
          new Error(
            'dbus-native: this bus never completed Hello (' +
              self.helloError.message +
          ...
          function (methodReturnResult) {
            ...
            try {
              if (
                ...
                // If signature has multiple top-level types and the result
                // is an array with the correct length, use it directly;
                // otherwise wrap the result in an array so it is treated as
                // a single return value.
                var isMultipleReturns = false;
                if (Array.isArray(methodReturnResult)) {
                  var typeCount = parseSignature(resultSignature).length;
                  isMultipleReturns =
                    typeCount > 1 && methodReturnResult.length === typeCount;
                }
                methodReturnReply.body = isMultipleReturns
                  ? methodReturnResult
                  : [methodReturnResult];
              }
  ...
  if (opts.direct !== true) {
    this.invokeDbus({ member: 'Hello' }, function (err, name) {
      if (err) {
        // This callback runs from the connection's 'message' listener, so throwing
        // here escaped as an uncaughtException and took the host process down - and
        // it did so even when the caller had an 'error' listener on the connection,
        // because a throw out of an emit() cannot be caught by the caller's await.
        // A bus that refuses Hello (AccessDenied on a locked-down system bus, say)
        // has to be reportable rather than fatal.
        var helloError = new Error(
          (err.name || 'Error') + ': ' + (err.message || err)
        );
        // Recorded as well as reported: surviving the refusal is not the same as
        // pretending it did not happen, and self.name stays undefined either way.
        self.helloError = helloError;
        if (self.connection.listenerCount('error') > 0) {
          self.connection.emit('error', helloError);
        } else {
          console.warn('dbus-native: ' + helloError.message);
        }
        return;
      }
      self.name = name;
