...
module.exports = function bus(conn, opts) {
  ...
  if (opts.direct !== true) {
    this.invokeDbus({ member: 'Hello' }, function (err, name) {
      if (err) {
        // Reported rather than thrown - see report-error.js. A bus that refuses Hello
        // (AccessDenied on a locked-down system bus, say) has to be survivable.
        ...
        reportReplyError(self.connection, err);
        return;
      }
      self.name = name;
    });
