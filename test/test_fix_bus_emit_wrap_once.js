...
describe('bus exportInterface wraps obj.emit only once across re-exports', function () {
  ...
  it('re-exporting the same (iface, path) replaces the entry instead of duplicating dispatch', function () {
    var bb = makeBus();
    var obj = new EventEmitter();
    bb.b.exportInterface(obj, '/p/a', pingedIface());
    // export the same iface on the same path again (e.g. after updating descriptor)
    bb.b.exportInterface(obj, '/p/a', pingedIface());
    bb.conn.outbox.length = 0;
    obj.emit('Pinged', 'hello');
    var pingedMessages = bb.conn.outbox.filter(function (m) {
      return m.member === 'Pinged';
    });
    assert.equal(
      pingedMessages.length,
      1,
      'signal must be dispatched once, not duplicated per re-export'
    );
    assert.equal(obj.__dbusInterfaces.length, 1);
  });
});
